...
function legend() {
    ...
    overlays.getLayers().getArray().slice().forEach(layer => {
        ...
        element.appendChild(head);
        var img = new Image();
        img.src = "http://localhost:8084/geoserver/sbc/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" + layer.get('title');
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8084/geoserver/sbc/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8084/geoserver/sbc/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
function wms_layers() {
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:8084/geoserver/sbc/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
